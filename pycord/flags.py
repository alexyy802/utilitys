...
The pycord cog base, which contains most of the actual functionality of pycord.
...
class Flag:
    """
    Dataclass that represents a pycord flag state. Only for internal use.
    """
    ...
    def resolve(self, flags):  # pylint: disable=too-many-return-statements
        ...
        env_value = os.getenv(f"pycord_{self.name}", "").strip()
...
class Flags(metaclass=FlagMeta):  # pylint: disable=too-few-public-methods
    """
    The flags for pycord.

    You can override these either through your environment, e.g.:
        export pycord_HIDE=1
    Or you can override them programmatically:
        pycord.Flags.HIDE = True
    """

    # Flag to indicate the pycord base command group should be hidden
    HIDE: bool
    ...
    # The scope prefix, i.e. the prefix that appears before pycord's builtin variables in REPL sessions.
