...
class MusicController:
    ...
    async def controller_loop(self):
        ...
        player = self.bot.audio.get_player(self.guild_id)
...
class Music(commands.Cog):
    ...
    async def start_nodes(self):
        ...
        node = await self.bot.audio.initiate_node(host='x.x.x.x',
                                                     port=2333,
    ...
    async def on_event_hook(self, event):
        """Node hook callback."""
        if isinstance(event, (audio.TrackEnd, audio.TrackException)):
            controller = self.get_controller(event.player)
            controller.next.set()

    def get_controller(self, value: Union[commands.Context, audio.Player]):
    ...
    async def play(self, ctx, *, query: str):
        ...
        tracks = await self.bot.audio.get_tracks(f'{query}')
    ...
    async def info(self, ctx):
        ...
        fmt = f'**audio:** `{audio.__version__}`\n\n' \
              f'Connected to `{len(self.bot.audio.nodes)}` nodes.\n' \
              f'Best available Node `{self.bot.audio.get_best_node().__repr__()}`\n' \
              f'`{len(self.bot.audio.players)}` players are distributed on nodes.\n' \
